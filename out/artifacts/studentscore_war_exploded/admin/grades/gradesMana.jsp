...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table width='98%'  border='0' align="center">
			  <tr>
			    <td >
			<form action="<%=path%>/grades?type=gradesMana" name="formAdd" method="post">
			      <c:if test="${sessionScope.role==1||sessionScope.role==2}"> <input type="button" value="添加" style="width: 80px;" onclick="gradesAdd()" /> </c:if>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			      ...
			      <input  type="submit" value="搜索" style="width:40px;"  />  支持学号 姓名 课程 学年的模糊搜索
          </form>	
				...
				<c:if test="${(sessionScope.role==1||sessionScope.role==2)||(grades.stu_code==sessionScope.admin.userName)}">
                <tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='red';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22">
					...
					<td bgcolor="#FFFFFF" align="center">
						<c:if test="${sessionScope.role==1||sessionScope.role==2}"><input type="button" value="删除" onclick="gradesDel(${grades.id})"/> </c:if>
					</td>
				</tr>
                </c:if>
				</c:forEach>
			 ...
			 <TR align="right">
              <TD ><form action="" method="post" name="formpage">
                  ...
                  <!--//用于给上面javascript传值-->
                  <input type="hidden" name="jumpurl" value="<%=path%>/grades?type=gradesMana&<%if(request.getParameter("name")!=null){out.print("name="+request.getParameter("name")+"&");} %>" />
                  <!--//用于给上面javascript传值--> 
